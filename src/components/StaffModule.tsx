...
export const StaffModule: React.FC<StaffModuleProps> = ({ users, setUsers, shifts, setShifts }) => {
  const [activeTab, setActiveTab] = useState<'list' | 'shifts'>('list');

  const staff = users.filter(u => u.role === 'admin' || u.role === 'staff' || u.role === 'manager');
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={handleAddStaff} className="bg-yellow-500 text-black px-4 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-yellow-400 transition-all">
          ...
          {staff.map((member) => (
            <div key={member.id} className="bg-zinc-900/40 rounded-2xl border border-zinc-800 p-6 hover:border-zinc-700 transition-all">
              <div className="flex items-start justify-between mb-6">
                ...
                <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase ${member.role === 'admin' ? 'bg-red-500/10 text-red-500 border border-red-500/20' : member.role === 'manager' ? 'bg-purple-500/10 text-purple-500 border border-purple-500/20' : 'bg-blue-500/10 text-blue-500 border border-blue-500/20'}`}>
                  {member.role === 'admin' ? 'Quản trị' : member.role === 'manager' ? 'Quản lý' : 'Nhân viên'}
                </span>
              </div>
              ...
              <div className="flex gap-2 pt-4 border-t border-zinc-800">
                ...
                <button onClick={() => handleDeleteStaff(member.id)} className="px-4 py-2 bg-red-500/10 text-red-500 rounded-xl text-xs font-bold hover:bg-red-500/20 transition-colors">Xóa</button>
              </div>
            </div>
          ))}
        </div>
                  ...
                  <tr key={shift.id} className="hover:bg-zinc-800/30 transition-colors">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-white text-sm">{staffMember?.name || 'Đã xóa'}</span>
                      </div>
                    </td>
                    <td className="p-4">
                      <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase ${shift.type === 'morning' ? 'bg-yellow-500/10 text-yellow-500' : shift.type === 'afternoon' ? 'bg-orange-500/10 text-orange-500' : 'bg-purple-500/10 text-purple-500'}`}>
