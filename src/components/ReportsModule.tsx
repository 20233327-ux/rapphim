...
export const ReportsModule: React.FC<ReportsModuleProps> = ({ movies, bookings }) => {
  ...
  const movieStats = movies.map(movie => {
    ...
    const tickets = movieBookings.length;
    return {
      id: movie.id,
      title: movie.title,
      ...
      revenue,
      tickets,
      occupancy: Math.min(Math.round((tickets / 100) * 100), 100) // Mock occupancy
    };
  }).sort((a, b) => b.revenue - a.revenue);

  const movieData = movieStats.slice(0, 4).map(s => ({
    name: s.title,
    value: totalRevenue > 0 ? Math.round((s.revenue / totalRevenue) * 100) : 0
  }));

  const revenueData = [
    { name: 'Thứ 2', revenue: totalRevenue * 0.1, tickets: Math.floor(totalTickets * 0.1) },
    { name: 'Thứ 3', revenue: totalRevenue * 0.12, tickets: Math.floor(totalTickets * 0.12) },
    { name: 'Thứ 4', revenue: totalRevenue * 0.08, tickets: Math.floor(totalTickets * 0.08) },
    { name: 'Thứ 5', revenue: totalRevenue * 0.15, tickets: Math.floor(totalTickets * 0.15) },
    { name: 'Thứ 6', revenue: totalRevenue * 0.2, tickets: Math.floor(totalTickets * 0.2) },
    { name: 'Thứ 7', revenue: totalRevenue * 0.25, tickets: Math.floor(totalTickets * 0.25) },
    { name: 'Chủ nhật', revenue: totalRevenue * 0.3, tickets: Math.floor(totalTickets * 0.3) },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Tổng doanh thu', value: `${totalRevenue.toLocaleString()}đ`, icon: DollarSign, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
          { label: 'Tổng vé bán ra', value: totalTickets.toLocaleString(), icon: Ticket, color: 'text-yellow-400', bg: 'bg-yellow-500/10' },
          { label: 'Khách hàng mới', value: '+12', icon: Users, color: 'text-blue-400', bg: 'bg-blue-500/10' },
          { label: 'Tỷ lệ tăng trưởng', value: '5.2%', icon: TrendingUp, color: 'text-purple-400', bg: 'bg-purple-500/10' },
        ].map((stat, i) => (
      ...
      <div className="bg-zinc-900/40 rounded-2xl border border-zinc-800 overflow-hidden">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-zinc-800">
            {movieStats.slice(0, 3).map((movie, i) => (
              <tr key={movie.id} className="hover:bg-zinc-800/30 transition-colors">
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    {movie.posterUrl && <img src={movie.posterUrl} alt="" className="w-8 h-12 rounded object-cover" referrerPolicy="no-referrer"/>}
                    <span className="font-semibold text-white text-sm">{movie.title}</span>
                  </div>
                </td>
                <td className="p-4 text-zinc-400 text-sm">{movie.tickets.toLocaleString()} vé</td>
                <td className="p-4 text-white font-bold text-sm">{movie.revenue.toLocaleString()}đ</td>
                <td className="p-4">
                  <div className="w-full max-w-[100px] h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                    <div className="h-full bg-yellow-500" style={{ width: `${movie.occupancy}%` }}/>
                  </div>
                </td>
